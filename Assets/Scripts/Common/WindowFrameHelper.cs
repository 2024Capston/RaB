...
public class WindowFrameHelper : MonoBehaviour
{
    ...
    {
        _top.localScale = new Vector3(_defaultScale * 2f, 1f / transform.localScale.y, 1f / transform.localScale.z);
        _top.position = transform.position + transform.rotation * Vector3.up * (_defaultScale * transform.localScale.y - 0.5f);

        _bottom.localScale = new Vector3(_defaultScale * 2f, 1f / transform.localScale.y, 1f / transform.localScale.z);
        _bottom.position = transform.position - transform.rotation * Vector3.up * (_defaultScale * transform.localScale.y - 0.5f);

        _left.localScale = new Vector3(1f / transform.localScale.x, _defaultScale * 2f, 1f / transform.localScale.z);
        _left.position = transform.position - transform.rotation * Vector3.right * (_defaultScale * transform.localScale.x - 0.5f);

        _right.localScale = new Vector3(1f / transform.localScale.x, _defaultScale * 2f, 1f / transform.localScale.z);
